...
// ... createLawsuit e getLawsuitById continuam iguais ao passo 14 ...
...
export async function batchImportLawsuits(req: Request<unknown, unknown, BatchImportBody>, res: Response) {
  ...
  for (const item of items) {
    ...
    // Usamos `returnDocument: 'after'` para evitar warnings e garantir o retorno tipado.
    ...
    // Se a data de criação for idêntica à de atualização, indica que o registro acabou de nascer
    const wasNew = lawsuit.createdAt?.getTime() === lawsuit.updatedAt?.getTime();

    await enqueueLawsuitSync({ lawsuitId: String(lawsuit._id), cnjNumber: lawsuit.cnjNumber });

    results.push({ cnjNumber, lawsuitId: String(lawsuit._id), wasNew, enqueued: true });
  }
...
> 💡 **As três interfaces novas, em uma frase cada.** `BatchImportItem` é o formato de **um item** da lista que a pessoa manda no corpo da requisição — o mesmo par `{ cnjNumber, clientId }` que `createLawsuit` (passo 14) já usava para um processo só, só que aqui vem repetido dentro de um array. `BatchImportBody` é só isso envolvido no formato que o Express espera para o corpo inteiro: `{ items: BatchImportItem[] }`. `BatchImportResultItem` é o formato de **cada item da resposta** — não confunda com `BatchImportItem` (que é de entrada): este já tem o `lawsuitId` gerado, `wasNew` (se o processo acabou de ser criado ou já existia) e `enqueued` (confirmando que o job entrou na fila). Nenhuma das três vira coleção nem model — são só formas de descrever, em tipo, o que entra e o que sai deste endpoint.
> 💡 **É na linha `await enqueueLawsuitSync(...)` que o processo entra na fila.** Cada volta do `for` chama essa função (criada no passo 17) uma vez — é o lado **produtor** da fila `lawsuit-sync`: ela só empacota `{ lawsuitId, cnjNumber }` e manda para o Redis via BullMQ, sem consultar o tribunal nem gravar nada. Quem efetivamente consome o job — consulta o "tribunal" simulado, grava as movimentações — é o Worker, do próximo passo (20). Até lá, o job só fica esperando na fila.
...
- **Mongoose — `Model.findOneAndUpdate()` (incluindo `upsert` e `rawResult`)**: https://mongoosejs.com/docs/api/model.html#Model.findOneAndUpdate()
- **MDN — código de status HTTP 202 Accepted**: https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/202
- **BullMQ — adicionando jobs (`Queue.add`, `jobId`)**: https://docs.bullmq.io/guide/queues/adding-jobs
